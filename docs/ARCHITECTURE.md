...
Phase 3A adds macOS-only `GOCompanionCapture` contracts, `MacCaptureAdapter`, and the `CaptureDiagnostic` executable. Only the adapter imports ScreenCaptureKit. Phase 3B adds the separate `GOCompanionScreenAnalysis` module, which consumes a bounded platform-neutral RGB frame and reports local screen-family classification/evidence; it does not create observations or collection facts. The diagnostic reports conservative raw labels and separate stable labels with bounded continuity. Phase 3C adds platform-neutral `GOCompanionExtraction` for targeted Detail/Appraisal observations, viewport geometry, and in-memory temporary specimen walkthroughs; `MacRecognitionAdapter` alone imports Vision. Neither module writes a permanent collection record. Dale manually live validated capture, classification, and a 12-Pokémon extraction walkthrough. See `SCANNING.md`.
...
The Phase 3C diagnostic receives frames from the chosen adapter, classifies screen type, creates field-level Detail/Appraisal observations, and aggregates them into temporary per-Pokémon sessions within an in-memory walkthrough. Ambiguous paging evidence is quarantined; corroborated new specimens receive new temporary UUIDs. No reconciliation, permanent collection write, or recommendation follows from this diagnostic. Later application use cases may reconcile accepted candidates against collection records and derive recommendations from facts, knowledge versions, and preferences.
