...
Phase 3A adds macOS-only `GOCompanionCapture` contracts, `MacCaptureAdapter`, and the `CaptureDiagnostic` executable. Only the adapter imports ScreenCaptureKit. Phase 3B adds the separate `GOCompanionScreenAnalysis` module, which consumes a bounded platform-neutral RGB frame and reports local screen-family classification/evidence; it does not create observations or collection facts. The diagnostic reports conservative raw labels and separate stable labels with bounded continuity. Dale manually live validated both selected-window capture and Phase 3B classification/continuity. Screen-specific field extraction remains future work. See `SCANNING.md`.
